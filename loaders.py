...
def get_de_loader(path, dur, subject):
        ...
        segs = 60 - dur + 1
        eeg_samples = np.zeros((32,40*segs,32,4*dur))
        eeg_labels = np.zeros((32,40*segs,5))
        for s in range(32):
        ...
        eeg, lab = {}, {}
        eeg['train'] = X_train.reshape(27*40*segs,32,4*dur)
        eeg['valid'] = X_valid.reshape(4*40*segs,32,4*dur)
        eeg['test']  = X_test.reshape(40*segs,32,4*dur)
        lab['train'] = y_train.reshape(27*40*segs,-1)
        lab['valid'] = y_valid.reshape(4*40*segs,-1)
        lab['test']  = y_test.reshape(40*segs,-1)
...
def get_crs_loader(path, dur, subject):
        ...
        for fold, (train_index, test_index) in enumerate(s.split(eeg_samples, eeg_labels)):
                ...
                eeg['train{}'.format(fold)] = X_train.reshape(32*segs,32,4*dur)
                eeg['test{}'.format(fold)]  = X_test.reshape(8*segs,32,4*dur)
                lab['train{}'.format(fold)] = y_train.reshape(32*segs,-1)
                lab['test{}'.format(fold)]  = y_test.reshape(8*segs,-1)
