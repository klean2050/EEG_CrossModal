...
def get_de_loader(path, dur, subject, num_tracks=34):
    ...
    lst = list(range(32))
    lst.remove(subject)
    X_train = eeg_samples[lst]
    X_test = eeg_samples[subject]
...
def get_crs_loader(path, dur, subject):
    ...
    for fold, (train_index, test_index) in enumerate(s.split(eeg_samples, to_stratify[:, 0])):
        ...
        X_test = eeg_samples[test_index]
        y_train = eeg_labels[train_index]
        y_test = eeg_labels[test_index]
        ...
        lab["test{}".format(fold)] = y_test.reshape(y_test.shape[0] * segs, -1)

    for x in eeg.keys():
        mus[x], mlab[x] = make_pairs(eeg[x], lab[x], mus_samples, mus_labels, dur)
...
def get_loader(path, mode, batch_size, dur, subject="0"):
    ...
    if mode == "eeg":
        ...
        dataset = {
            x: CROSSDataSet(eeg=eeg[x], mus=mus[x], elabels=elabels[x], mlabels=mlabels[x])
            for x in eeg.keys()
        }

    dataloader = {
        x: DataLoader(dataset[x], batch_size=batch_size, num_workers=0, shuffle=True) for x in dataset.keys()
    }
    return dataloader
